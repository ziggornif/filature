...
**Category:** feature
**Summary:** Stand up the app substrate — boots (config → PostgreSQL pool → embedded migrations), embeds all assets, and serves the app-shell in light/dark themes and en/fr locales — so domain slices have something to plug into. (Persistence is PostgreSQL per [ADR-0003](../adr/0003-postgresql-persistence.md), which superseded the original SQLite choice during PR-1 review.)
...
- Starting the binary: loads configuration from a TOML file **with environment-variable overrides**, connects a **PostgreSQL** pool, runs **embedded** migrations at startup, and serves HTTP on the configured bind address.
- All runtime assets are **embedded in the binary**: HTML templates, static files (CSS), i18n catalogs, and migrations (htmx is CDN-loaded). No filesystem dependency at runtime beyond the optional config file; the database is the one external service ([ADR-0003](../adr/0003-postgresql-persistence.md)).
...
- A **persistence entry point** that, given a Postgres database URL, returns a ready connection pool with all embedded migrations applied. This is the seam later slices' SPI adapters (`MaterialRepository`, `SpoolRepository`, …) build on.
...
- [ ] An integration test opens a **real PostgreSQL** pool (via testcontainers) and confirms migrations ran.
...
- [ ] Running the binary against a real Postgres serves the shell; toggling OS light/dark restyles it; a `lang=fr` cookie switches labels (manual smoke, per the verify skill).
